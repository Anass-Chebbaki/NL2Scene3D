...
from nl2scene3d.utils.geometry import collision_score, object_above_floor
...
def apply_rigid_transform(
    ...
    La Z del figlio NON viene mai modificata: usa `original_z` se fornita,
    altrimenti mantiene la Z corrente del figlio invariata.
    Solo il piano XY viene trasformato rigidamente.
    ...
    # Nuova posizione assoluta del figlio (solo XY!)
    child.transform.location[0] = new_parent_loc[0] + rot_x
    child.transform.location[1] = new_parent_loc[1] + rot_y
    # Z ASSOLUTAMENTE INVARIATA: usa il valore originale se disponibile
    if original_z is not None:
        child.transform.location[2] = original_z
    # altrimenti child.transform.location[2] rimane inalterato
...
class SceneRandomizer:
    ...
    def randomize(self, state: SceneState) -> SceneState:
        ...
        for root in roots:
            ...
            # Inizializziamo con la posizione originale come baseline sicura per evitare AssertionError
            best_obj = root.copy()
            ...
            # best_obj è garantito essere non-None
            ...
            # Clampa l'intera posizione finale
            ...
            # Lock assoluto della Z del root: usa sempre il valore originale dalla scena
            ...
            # Sposta i figli con trasformazione rigida; Z di ogni figlio è invariata
            self._move_children(
                root_name=root.name,
                old_loc=old_loc,  # usa le coordinate ORIGINALI del root salvate prima della modifica
                old_rz=old_rz,
                new_loc=new_loc,
                new_rz=new_rz,
                by_name=by_name,
    ...
    def _group_collision_score(
        ...
        La Z è già bloccata al valore originale da apply_rigid_transform,
        quindi NON serve un check pavimento qui — sarebbe eccessivamente restrittivo.
        """
        ...
        def _collect_and_score_children(parent_name: str, current_parent_loc: list[float], current_parent_rz: float):
            ...
            for child_name in orig_parent.children:
                ...
                # Crea una copia temporanea del figlio spostata (XY rigido, Z ORIGINALE)
                ...
                apply_rigid_transform(
                    ...
                    original_z=orig_child.transform.location[2],  # Z INVARIATA
                )

                # Collisione per questo figlio contro altri oggetti
                total_score += collision_score(
                    moved_child,
                    placed_objects,
                    wall_margin=self.config.wall_margin,
                ...
                # Verifica bounds per questo figlio
                if bounds is not None and not bounds.contains_aabb(
                    moved_child.transform.aabb_xy(margin=0.0), margin=self.config.wall_margin
                ):
                    total_score += 100.0

                # Ricorsione per i figli dei figli (usa la posizione ORIGINALE del child come old)
    ...
    def _move_children(
        ...
        """
        Sposta ricorsivamente tutti i discendenti del root con trasformazione rigida XY.
        La Z di ogni figlio viene ripristinata al valore originale (dalla scena pre-randomizzazione)
        per garantire che nessun figlio possa mai cambiare quota.
        """
        ...
        for child_name in root_obj.children:
            ...
            # Posizione originale del figlio (dalla scena pre-randomizzazione)
            original_child_z = None
            orig_child_loc = old_loc  # fallback se orig_by_name non disponibile
            ...
            # Ricorsione per i nipoti: usa la pos ORIGINALE del child come old_loc
            self._move_children(
                child_name,
                orig_child_loc,  # posizione originale del child, NON quella appena calcolata
                orig_child_rz,
