...
def _get_config_dir() -> Path:
    ...
    # 1. Cerca nella sottocartella 'config' interna al pacchetto (Addon/Bundled)
    pkg_config = Path(__file__).parent / "config"
    if pkg_config.exists():
        return pkg_config
    
    # 2. Cerca nella root del progetto (Standalone development)
...
def _load_toml_config() -> dict[str, Any]:
    """
    Carica la configurazione dal file settings.toml.
    ...
    """
    config_path = _get_config_dir() / "settings.toml"
